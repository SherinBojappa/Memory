...
with open('memory_retention_raw.csv', 'w') as csvfile:
    fieldnames = ['index', 'seq_len', 'seq', 'dense_encoded_seq', 'token_repeated', 'dense_encoded_query_token', 'rep_token_first_pos', 'query_token', 'target_val' ]
    writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
    writer.writeheader()
    total_samples = len(x)
    for sample in range(total_samples):
        writer.writerow({'index': sample, 'seq_len': sequence_len[sample], 'seq':raw_sequence[sample][:-1],
                          'dense_encoded_seq':x[sample][:-1], 'token_repeated':token_repeated[sample],
                         'dense_encoded_query_token':x[sample][-1], 'rep_token_first_pos': pos_first_token[sample], 'query_token':raw_sequence[sample][-1],
